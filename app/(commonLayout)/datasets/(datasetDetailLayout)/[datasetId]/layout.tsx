  ...
  return (
    <Link className={classNames(s.itemWrapper, 'px-2', isMobile && 'justify-center')} href={`/app/${detail?.id}/overview`}>
      <div className={classNames(s.iconWrapper, 'mr-0')}>
        ...
        {type === 'app' && (
          <span className='absolute bottom-[-2px] right-[-2px] w-3.5 h-3.5 p-0.5 bg-white rounded border-[0.5px] border-[rgba(0,0,0,0.02)]'>
            {detail.mode === 'advanced-chat' && (
              <ChatBot className='w-2.5 h-2.5 text-[#7C3AED]' />
            )}
            {detail.mode === 'agent-chat' && (
              <CuteRobote className='w-2.5 h-2.5 text-primary-600' />
            )}
            {detail.mode === 'chat' && (
              <ChatBot className='w-2.5 h-2.5 text-[#7C3AED]' />
            )}
...
const TargetIcon = ({ className }: SVGProps<SVGElement>) => {
  return <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg" className={className ?? ''}>
    <g clipPath="url(#clip0_4610_6951)">
      <path d="M10.6666 5.33325V3.33325L12.6666 1.33325L13.3332 2.66659L14.6666 3.33325L12.6666 5.33325H10.6666ZM10.6666 5.33325L7.9999 7.99988M14.6666 7.99992C14.6666 11.6818 11.6818 14.6666 7.99992 14.6666C4.31802 14.6666 1.33325 11.6818 1.33325 7.99992C1.33325 4.31802 4.31802 1.33325 7.99992 1.33325M11.3333 7.99992C11.3333 9.84087 9.84087 11.3333 7.99992 11.3333C6.15897 11.3333 4.66659 9.84087 4.66659 7.99992C4.66659 6.15897 6.15897 4.66659 7.99992 4.66659" stroke="#474E54" strokeWidth="1.25" strokeLinecap="round" strokeLinejoin="round" />
    </g>
    <defs>
...
const DatasetDetailLayout: FC<IAppDetailLayoutProps> = (props) => {
  ...
  useEffect(() => {
    if (datasetRes)
      document.title = `${datasetRes.name || 'Dataset'} - Emergen`
  }, [datasetRes])
  ...
  return (
    <div className='grow flex overflow-hidden border-t-[0.5px] border-gray-300'>
      ...
      <DatasetDetailContext.Provider value={{
        ...
        <div className="bg-gray-25 grow overflow-hidden">{children}</div>
