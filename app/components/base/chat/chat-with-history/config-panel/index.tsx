...
const ConfigPanel = () => {
  ...
  return (
    <div className='flex flex-col max-h-[80%] w-full max-w-[720px] px-4'>
      <div
        className={`
          grow rounded-xl overflow-y-auto
          ${showConfigPanelBeforeChat && 'border-[0.5px] border-gray-100 shadow-2xl'}
          ${!showConfigPanelBeforeChat && collapsed && 'border border-violet-100'}
          ${!showConfigPanelBeforeChat && !collapsed && 'border-[0.5px] border-gray-100 shadow-2xl'}
        `}
      >
        <div
          className={`
            flex flex-wrap px-6 py-6 rounded-t-3xl border-b-[0.5px] bg-white
            ${isMobile && '!px-6 !py-5'}
          `}
        >
          {
            showConfigPanelBeforeChat && (
              <>
                <div className='flex items-center text-xl !text-lg font-semibold text-gray-900 gap-4 leading-6'>
                  <AppIcon
                    size='small'
                    className='!bg-transparent'
                    iconClassName="text-primary-600 w-6"
                  />
                  {appData?.site.title}
                </div>
                {
          ...
          }
          {
            !showConfigPanelBeforeChat && collapsed && (
              <>
                <Star06 className='mr-1 mt-1 w-4 h-4 text-violet-600' />
                <div className='grow py-[3px] text-[13px] text-violet-600 leading-[18px] font-medium'>
      ...
      </div>
      {
        showConfigPanelBeforeChat && (site || customConfig) && (
          <div className='mt-4 flex flex-wrap justify-between items-center py-2 text-xs text-gray-500'>
            {site?.privacy_policy
              ...
              customConfig?.remove_webapp_brand
                ...
                : (
                  <div className={`flex items-center justify-end ${isMobile && 'w-full'}`}>
                    <div className='flex items-center pr-3 space-x-3 opacity-50'>
