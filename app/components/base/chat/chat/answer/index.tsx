  ...
  return (
    <div className='flex mb-2 last:mb-0'>
      <div className='shrink-0 relative w-10 h-10'>
        {
          answerIcon || (
            <div className='flex items-center justify-center w-full h-full rounded-full bg-[#681DEA] text-xl'>
              <RiRobot2Fill className='w-5 h-5 text-[#FCFDFE]' />
            </div>
          )
        }
        {
      ...
      <div className='chat-answer-container group grow w-0 ml-4' ref={containerRef}>
        <div className={`group relative pr-10 ${chatAnswerContainerInner}`}>
          <AnswerTriangle className='absolute -left-2 top-0 w-2 h-3 text-gray-300' />
          <div
            ref={contentRef}
            className={`
              relative inline-block px-4 py-4 max-w-full bg-gray-300 rounded-b-2xl rounded-tr-2xl text-sm text-gray-900
              ${workflowProcess && 'w-full'}
            `}
          >
            {annotation?.id && (
              <div
                className='absolute -top-3.5 -right-3.5 box-border flex items-center justify-center h-7 w-7 p-0.5 rounded-lg bg-white cursor-pointer text-[#681CEA] shadow-md group-hover:hidden'
              >
