    ...
    <>
      ...
      >
        ...
        {!isOpeningStatement && (showPromptLog || config?.text_to_speech?.enabled) && (
          <div className='hidden group-hover:flex items-center w-max h-[28px] p-0.5 rounded-lg bg-white border-[0.5px] border-gray-100 shadow-lg shrink-0'>
            {showPromptLog && (
              <Log logItem={item} />
            )}
            {(config?.text_to_speech?.enabled) && (
              <>
                <div className='mx-1 w-[1px] h-[14px] bg-gray-300'/>
                <AudioBtn
                  id={id}
                  value={content}
                  noCache={false}
        ...
        )}
        {
          !positionRight && annotation?.id && (
            <div
              className='relative box-border flex items-center justify-center h-7 w-7 p-0.5 rounded-lg bg-white cursor-pointer text-[#681CEA] shadow-md group-hover:hidden'
            >
              <div className='p-1 rounded-lg bg-[#F8F4FE] '>
        ...
        }
        {
          config?.supportFeedback && !localFeedback?.rating && onFeedback && !isOpeningStatement && (
            <div className='hidden group-hover:flex ml-1 shrink-0 items-center px-0.5 bg-white border-[0.5px] border-gray-100 shadow-lg text-gray-500 rounded-lg'>
              <TooltipPlus popupContent={t('appDebug.operation.agree')}>
                <div
                  className='flex items-center justify-center mr-0.5 w-6 h-6 rounded-md hover:bg-gray-300 hover:text-gray-800 cursor-pointer'
              ...
              <TooltipPlus popupContent={t('appDebug.operation.disagree')}>
                <div
                  className='flex items-center justify-center w-6 h-6 rounded-md hover:bg-gray-300 hover:text-gray-800 cursor-pointer'
