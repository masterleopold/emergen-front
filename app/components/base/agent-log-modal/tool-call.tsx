...
const ToolCallItem: FC<Props> = ({ toolCall, isLLM = false, isFinal, tokens, observation, finalAnswer }) => {
  ...
  return (
    <div className={cn('py-1')}>
      <div className={cn('group transition-all bg-white border-[0.5px] border-gray-100 rounded-xl hover:shadow-md')}>
        <div
          className={cn(
        ...
        {!collapseState && (
          <div className='pb-2'>
            <div className={cn('px-[10px] py-1')}>
              {toolCall.status === 'error' && (
                <div className='px-3 py-[10px] bg-[#fef3f2] rounded-lg text-xs leading-[18px] text-[#d92d20]'>{toolCall.error}</div>
              )}
            </div>
            {toolCall.tool_input && (
              <div className={cn('px-[10px] py-1')}>
