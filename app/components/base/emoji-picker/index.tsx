...
const EmojiPicker: FC<IEmojiPickerProps> = ({
  ...
  >
    <div className='flex flex-col items-center w-full p-3'>
      <div className="relative w-full">
        <div className="absolute inset-y-0 left-0 flex items-center pl-2 pointer-events-none">
          <MagnifyingGlassIcon className="w-4 h-4 text-gray-500" aria-hidden="true" />
        </div>
        <input
          type="search"
          id="search"
          className='block w-full h-10 px-3 pl-10 text-[13px] leading-6 font-normal bg-gray-100 rounded-lg placeholder:text-gray-500'
    ...
    <div className="w-full max-h-[200px] overflow-x-hidden overflow-y-auto px-3">
      {isSearching && <>
        <div key={'category-search'} className='flex flex-col'>
          <p className='font-medium uppercase text-xs text-[#202429] mb-1'>Search</p>
      ...
      {categories.map((category, index: number) => {
        return <div key={`category-${index}`} className='flex flex-col'>
          <p className='font-medium uppercase text-xs text-[#202429] mb-1'>{category.id}</p>
          <div className='w-full h-full grid grid-cols-8 gap-1'>
    ...
    <div className={cn('p-3 ', selectedEmoji === '' ? 'opacity-25' : '')}>
      <p className='font-medium uppercase text-xs text-[#202429] mb-2'>Choose Style</p>
      <div className='w-full h-full grid grid-cols-8 gap-1'>
