    ...
    <div className={cn(s.item, isChosen && s.active)}>
      <div className='flex py-3 pl-3 pr-4' onClick={onChosen}>
        ...
        <div className='grow space-y-2'>
          ...
          <div className='leading-[18px] text-xs font-normal text-[#707B84]'>{description}</div>
        </div>
        {!noRadio && (
          <div className='shrink-0 flex items-center h-8 ml-3'>
            <div className={s.radio}></div>
          </div>
        )}
      </div>
      {((isChosen && chosenConfig) || noRadio) && (
        <div className={cn(chosenConfigWrapClassName, 'pt-3 px-14 pb-6 border-t-[0.5px] border-gray-300')}>
          {chosenConfig}
        </div>
      )}
    </div>
