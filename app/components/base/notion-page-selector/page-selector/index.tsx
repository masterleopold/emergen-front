...
const ItemComponent = ({ index, style, data }: ListChildComponentProps<{
  ...
  const renderArrow = () => {
    if (hasChild) {
      return (
        <div
          className={cn(s.arrow, current.expand && s['arrow-expand'], 'shrink-0 mr-1 w-5 h-5 hover:bg-gray-200 rounded-lg')}
  ...
  return (
    <div
      className={cn('group flex items-center pl-2 pr-[2px] rounded-md border-[0.5px] border-transparent hover:bg-gray-100 cursor-pointer', previewPageId === current.page_id && s['preview-item'])}
