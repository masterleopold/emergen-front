    ...
    <div className={cn(!trigger && !noLeft && 'left-[-8px]', 'relative')} ref={ref}>
      {trigger
        ...
        : (
          <div
            onClick={toggleShow}
            className={cn(showOption && 'bg-gray-300', 'flex items-center h-5 pl-1 pr-0.5 rounded-md text-xs font-semibold text-gray-700 cursor-pointer hover:bg-gray-300')}>
      ...
      {(showOption && !readonly) && (
        <div className={cn(popupClassName, 'absolute z-10 top-[24px] w-[120px]  p-1 border-[0.5px] border-gray-300 shadow-2xl rounded-lg bg-white')}>
          {list.map(item => (
            <div
              key={item.value}
              onClick={() => {
