          ...
          <div className='flex items-center !normal-case'>
            ...
            {(!isEditGroupName)
              ...
              : (
                <input
                  type='text'
                  className='h-6 px-1 rounded-lg bg-white border-[0.5px] border-gray-300 focus:outline-none'
                  // style={{
      ...
      operations={
        <div className='flex items-center h-6  space-x-2'>
          {payload.variables.length > 0 && (
            <div className='flex items-center h-[18px] px-1 border-[0.5px] border-black/8 rounded-[5px] text-xs font-medium text-gray-500 capitalize'>{payload.output_type}</div>
          )}
          {
            !readOnly
              ? <VarReferencePicker
