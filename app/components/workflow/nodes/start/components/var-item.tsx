...
const VarItem: FC<Props> = ({
  ...
  return (
    <div ref={ref} className='flex items-center h-8 justify-between px-2.5 bg-white rounded-lg border-[0.5px] border-gray-300 cursor-pointer hover:bg-gray-50'>
      ...
      <div className='shrink-0 ml-2 flex items-center'>
        {rightContent || (<>
          {(!isHovering || readonly)
            ...
            : (!readonly && (
              <>
                <div onClick={showEditVarModal} className='mr-1 p-1 rounded-md cursor-pointer hover:bg-gray-300'>
                  <Edit03 className='w-4 h-4 text-gray-500' />
                </div>
                <div onClick={onRemove} className='p-1 rounded-md cursor-pointer hover:bg-gray-300'>
