...
const NodePanel: FC<Props> = ({
  ...
  return (
    <div className={cn('px-4 py-1', className, hideInfo && '!p-0')}>
      <div className={cn('group transition-all bg-white border-[0.5px] border-gray-100 rounded-2xl hover:shadow-xl', hideInfo && '!rounded-lg')}>
        <div
          className={cn(
          ...
          <div className='pb-2'>
            ...
            {isIterationNode && !notShowIterationNav && (
              <div className='mt-2 mb-1 !px-2'>
                <div
                  className='flex items-center h-[34px] justify-between px-3 bg-gray-100 border-[0.5px] border-gray-300 rounded-lg cursor-pointer'
                  ...
                  {justShowIterationNavArrow
                    ...
                    : (
                      <div className='flex items-center space-x-1 text-[#7C3AED]'>
            ...
            <div className={cn('px-[10px] py-1', hideInfo && '!px-2 !py-0.5')}>
              {nodeInfo.status === 'stopped' && (
                <div className='px-3 py-[10px] bg-[#fffaeb] rounded-lg text-xs leading-[18px] text-[#dc6803]'>{t('workflow.tracing.stopBy', { user: nodeInfo.created_by ? nodeInfo.created_by.name : 'N/A' })}</div>
              )}
              {nodeInfo.status === 'failed' && (
                <div className='px-3 py-[10px] bg-[#fef3f2] rounded-lg text-xs leading-[18px] text-[#d92d20]'>{nodeInfo.error}</div>
              )}
            </div>
            {nodeInfo.inputs && (
              <div className={cn('px-[10px] py-1', hideInfo && '!px-2 !py-0.5')}>
