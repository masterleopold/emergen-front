...
const Apps = ({
  ...
  return (
    <div className={cn(
      'flex flex-col',
      pageType === PageType.EXPLORE ? 'h-full border-l-[0.5px] border-gray-300 pt-4 md:pt-6' : 'h-[calc(100%-56px)] bg-gray-25',
    )}>
      <div className={cn(
        'flex items-center mt-2',
        pageType === PageType.EXPLORE ? 'px-4 md:px-12' : 'px-4 md:px-8',
      )}>
        {pageType !== PageType.EXPLORE && (
          <>
            <AppTypeSelector value={currentType} onChange={setCurrentType} />
            <div className='mx-2 w-[1px] h-3.5 bg-gray-25'/>
          </>
        )}
        <Category
          list={categories}
      ...
      <div className={cn(
        'relative flex flex-1 pb-12 flex-col overflow-auto bg-gray-25 shrink-0 grow',
        pageType === PageType.EXPLORE ? 'mt-6' : 'mt-4 pt-2',
      )}>
        <nav
          className={cn(
            s.appList,
