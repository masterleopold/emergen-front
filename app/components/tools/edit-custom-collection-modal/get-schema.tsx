...
const GetSchema: FC<Props> = ({
  ...
  return (
    <div className='flex gap-1 justify-end relative w-[224px] items-center'>
        ...
        {showImportFromUrl && (
          <div className=' absolute left-[-35px] top-[26px] p-2 rounded-lg border-[0.5px] border-gray-300 bg-white shadow-lg focus-visible:outline-none'>
            <div className='relative'>
              <input
                type='text'
                className='w-[244px] h-8 pl-1.5 pr-[44px] overflow-x-auto border-[0.5px] border-gray-300 rounded-lg text-[13px]'
      ...
      <div className='relative' ref={showExamplesRef}>
        ...
        {showExamples && (
          <div className='absolute top-7 right-0 p-1 rounded-lg bg-white'>
            {examples.map(item => (
              <div
                key={item.key}
                onClick={() => {
