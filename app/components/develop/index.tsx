...
const DevelopMain = ({ appId }: IDevelopMainProps) => {
  ...
  return (
    <div className='relative flex flex-col h-full overflow-hidden'>
      <div className='flex items-center justify-between flex-shrink-0 px-6 border-b-[0.5px] border-solid py-4 border-b-gray-300'>
        <div className='text-lg font-medium text-gray-900'></div>
        <div className='flex items-center flex-wrap gap-y-1 gap-1'>
          <InputCopy className='flex-shrink-0 mr-1 w-52 sm:w-80' value={appDetail.api_base_url}>
            <div className={`ml-2 border-[0.5px] border-gray-300 border-solid flex-shrink-0 px-2 py-0.5 rounded-[6px] text-gray-500 text-[0.625rem] ${s.customApi}`}>
              {t('appApi.apiServer')}
            </div>
          </InputCopy>
          ...
          <SecretKeyButton className='flex-shrink-0 h-9' appId={appId} />
        </div>
      </div>
      <div className='px-4 sm:px-10 py-10 overflow-auto grow'>
