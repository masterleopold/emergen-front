...
function CodePanelHeader({ tag, label }: { tag: string; label: string }) {
  ...
  return (
    <div className="flex h-9 items-center gap-2 border-y-[0.5px] border-t-transparent border-b-white/7.5 bg-zinc-900 bg-white/2.5 px-4 dark:border-b-white/5 dark:bg-white/1">
      {tag && (
...
function CodeGroupHeader({ title, children, selectedIndex }: IChildrenProps) {
  ...
  return (
    <div className="flex min-h-[calc(theme(spacing.12)+1px)] flex-wrap items-start gap-x-4 border-b-[0.5px] border-zinc-700 bg-zinc-800 px-4 dark:border-zinc-800 dark:bg-transparent">
      {title && (
        <h3 className="pt-4 mr-auto text-xs font-semibold text-white">
          {title}
        </h3>
      )}
      {hasTabs && (
        <Tab.List className="flex gap-4 -mb-px text-xs font-medium">
          {Children.map(children, (child, childIndex) => (
            <Tab
              className={classNames(
                'border-b-[0.5px] py-3 transition focus:[&:not(:focus-visible)]:outline-none',
