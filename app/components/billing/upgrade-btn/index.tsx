...
const PlainBtn = ({ className, onClick }: { className?: string; onClick: () => void }) => {
  ...
  return (
    <div
      className={cn(className, 'flex items-center h-8 px-3 rounded-lg border-[0.5px] border-gray-300 bg-white cursor-pointer')}
      onClick={onClick}
    >
  ...
  return (
    <div
      className={cn(
        ...
        size === 'lg' ? 'h-10' : 'h-9',
        'relative flex items-center cursor-pointer border-[0.5px] rounded-[20px] border-[#0096EA] text-white',
      )}
      onClick={onClick}
    >
