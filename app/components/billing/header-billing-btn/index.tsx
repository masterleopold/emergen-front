...
const HeaderBillingBtn: FC<Props> = ({
  ...
  const classNames = (() => {
    ...
    if (type === Plan.team)
      return 'border-[#F8F4FE] hover:border-[#C7D7FE] bg-[#F8F4FE] text-[#3538CD]'
    return ''
  })()
  ...
  return (
    <div onClick={onClick} className={cn(classNames, 'flex items-center h-[22px] px-2 rounded-md border-[0.5px] text-xs font-semibold uppercase cursor-pointer')}>
      {name}
    </div>
  )
}
