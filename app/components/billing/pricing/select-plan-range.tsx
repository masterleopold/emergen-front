...
const ITem: FC<{ isActive: boolean; value: PlanRange; text: string; onClick: (value: PlanRange) => void }> = ({ isActive, value, text, onClick }) => {
  return (
    <div
      className={cn(isActive ? 'bg-[#7C3AED] text-white' : 'text-gray-900', 'flex items-center px-8 h-11 rounded-[32px] cursor-pointer text-[15px] font-medium')}
...
const SelectPlanRange: FC<Props> = ({
  ...
  return (
    <div>
      ...
      <div className='inline-flex relative p-1 rounded-full bg-[#F8F4FE] border-[0.5px] border-gray-300'>
