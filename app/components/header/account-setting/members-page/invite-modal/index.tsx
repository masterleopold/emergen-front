        ...
        <div>
          ...
          <Listbox value={role} onChange={setRole}>
            <div className="relative pb-6">
              <Listbox.Button className="relative w-full py-2 pl-3 pr-10 text-left bg-gray-200 !outline-none !border-none appearance-none text-sm text-gray-900 rounded-lg">
              ...
              <Transition
                ...
                leaveTo="opacity-0"
              >
                <Listbox.Options className="absolute w-full py-2 my-2 overflow-auto text-base bg-white rounded-2xl shadow-xl max-h-60 focus:outline-none sm:text-sm">
                  {InvitingRoles.map(role =>
                    <Listbox.Option
                      key={role.name}
                      className={({ active }) =>
                        `${active ? ' bg-gray-100 rounded-lg' : ' bg-transparent'}
                      ...
                      {({ selected }) => (
                        <div className='flex flex-row'>
                          ...
                          <div className=' flex flex-col flex-grow'>
                            ...
                            <span className={`${selected ? 'font-normal' : 'font-normal'} capitalize block text-gray-500`}>
                              {role.description}
                            </span>
                          </div>
                        </div>
