    ...
    <>
      <div
        className={classNames(
          'min-h-16 bg-gray-50 border-[0.5px] rounded-xl transition-colors',
          (withSwitch || !draftConfig.enabled) ? 'border-gray-300' : 'border-primary-400',
      ...
      >
        <div className='flex items-center px-[15px] py-3 gap-2 select-none'>
          <div className='grow-0 shrink-0 flex items-center justify-center w-8 h-8 text-primary-600 bg-primary-50 border-[0.5px] border-primary-100 rounded-lg'>
        ...
        {draftConfig.enabled && (
          <div className='flex flex-col gap-1 px-3 pb-3'>
            {draftConfig.configs.map((config, index) => {
              ...
              return (
                <div key={config.id || index} className='group flex items-center px-3 h-10 bg-white border-[0.5px] border-gray-300 rounded-lg shadow-xs'>
                  <div className='grow flex items-center'>
                    ...
                    {isProviderManaged && (
                      <span className='px-1 text-2xs uppercase text-gray-500 border-[0.5px] border-black/8 rounded-[5px]'>{t('common.modelProvider.providerManaged')}</span>
                    )}
                  </div>
                    ...
                    {!isProviderManaged && (
                      <>
                        <div className='flex items-center gap-1 opacity-0 transition-opacity group-hover:opacity-100'>
                          <span
                            className='flex items-center justify-center w-8 h-8 text-gray-500 bg-white rounded-lg transition-colors cursor-pointer hover:bg-gray-300'
                          ...
                          <span className='mr-2 h-3 border-r-[0.5px] border-r-gray-100' />
                        </div>
                      </>
                    )}
                    <Switch
        ...
        )}
        {
          draftConfig.enabled && draftConfig.configs.length < 2 && (
            <div className='flex items-center px-6 h-[34px] text-xs text-gray-700 bg-black/2 border-t-[0.5px] border-t-black/5'>
      ...
      {!modelLoadBalancingEnabled && !IS_CE_EDITION && (
        <GridMask canvasClassName='!rounded-xl'>
          <div className='flex items-center justify-between mt-2 px-4 h-14 border-[0.5px] border-gray-300 rounded-xl shadow-md'>
            <div
