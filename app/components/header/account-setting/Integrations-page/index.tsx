      ...
      <div className='mb-8'>
        <div className={titleClassName}>{t('common.integrations.connected')}</div>
        {
          integrates.map(integrate => (
            <div key={integrate.provider} className='mb-2 flex items-center px-3 py-2 bg-gray-100 rounded-xl'>
              <div className={classNames('w-8 h-8 mr-3 bg-white rounded-lg border-[0.5px] border-gray-100', s[`${integrate.provider}-icon`])} />
              <div className='grow'>
              ...
              </div>
              {
                !integrate.is_bound && (
                  <Link
                    className='flex items-center h-8 px-4 bg-white rounded-lg border-[0.5px] border-gray-300 text-xs font-medium text-gray-700 cursor-pointer'
                    href={integrate.link}
      ...
      {/* <div className='mb-8'>
        <div className={titleClassName}>Add a service </div>
        {
          services.map(service => (
            <div key={service.key} className='mb-2 flex items-center px-3 py-3 bg-gray-100 border-[0.5px] border-gray-300 rounded-xl'>
              <div className={classNames('w-8 h-8 mr-3 bg-white rounded-lg border-[0.5px] border-gray-100', s[`${service.key}-icon`])} />
              <div className='grow'>
