        ...
        hitHistoryList.length > 0
          ? (
            <div className='flex items-center space-x-1'>
              ...
              <div className='flex px-1.5 item-center rounded-md border-[0.5px] border-black/[8%] h-5 text-xs font-medium text-gray-500'>{total} {t(`appAnnotation.viewModal.hit${hitHistoryList.length > 1 ? 's' : ''}`)}</div>
            </div>
          )
    ...
    : (
      <div>
        ...
        {(total && total > APP_PAGE_LIMIT)
          ? <Pagination
            className="flex items-center w-full h-10 text-sm select-none mt-4"
          ...
          >
            <Pagination.PrevButton
              disabled={currPage === 0}
              className={`flex items-center mr-2 text-gray-500  focus:outline-none ${currPage === 0 ? 'cursor-not-allowed opacity-50' : 'cursor-pointer hover:text-gray-600 '}`} >
            ...
            <Pagination.NextButton
              ...
              className={`flex items-center mr-2 text-gray-500 focus:outline-none ${currPage === Math.ceil(total / APP_PAGE_LIMIT) - 1 ? 'cursor-not-allowed opacity-50' : 'cursor-pointer hover:text-gray-600 '}`} >
  ...
  return (
    <div>
      <Drawer
        ...
        foot={id
          ? (
            <div className='px-4 flex h-16 items-center justify-between border-t-[0.5px] border-gray-300 bg-gray-200 rounded-bl-xl rounded-br-xl leading-[18px] text-[13px] font-medium text-gray-500'>
              <div
