...
const Embedded = ({ siteInfo, isShow, onClose, appBaseUrl, accessToken, className }: Props) => {
  ...
  const onClickCopy = () => {
    copy(OPTION_MAP[option].getContent(appBaseUrl, accessToken, themeBuilder.theme?.primaryColor ?? '#681CEA', isTestEnv))
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-wrap items-center gap-y-2 space-x-3">
      ...
      <div className={cn('w-full bg-gray-100 rounded-lg flex-col justify-start items-start inline-flex',
        'mt-6')}>
        <div className="inline-flex items-center self-stretch justify-start gap-2 py-1 pl-3 pr-1 border-[0.5px] border-black rounded-tl-lg rounded-tr-lg bg-gray-50 border-opacity-5">
        ...
        <div className="flex items-start justify-start w-full gap-2 p-3 overflow-x-auto">
          <div className="grow shrink basis-0 text-slate-700 text-[13px] leading-tight font-mono">
            <pre className='select-text'>{OPTION_MAP[option].getContent(appBaseUrl, accessToken, themeBuilder.theme?.primaryColor ?? '#681CEA', isTestEnv)}</pre>
          </div>
        </div>
      </div>
    </Modal>
