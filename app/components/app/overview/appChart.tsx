...
const COLOR_TYPE_MAP = {
  purple: {
    lineColor: 'rgba(104, 28, 234, 1)',
    bgColor: ['rgba(104, 28, 234, 0.2)', 'rgba(104, 28, 234, 0.08)'],
  },
}
...
const Chart: React.FC<IChartProps> = ({
  ...
  const options: EChartsOption = {
    ...
    : `<span style='font-size:12px'>
                                  ...
                                  <span style='color:#681CEA'>~$${get(params.data, 'total_price', 0)}</span>
  ...
  return (
    <div className={`flex flex-col w-full px-6 py-5 border-[0.5px] rounded-xl border-gray-300 ${className ?? ''}`}>
      <div className='mb-3'>
        ...
        <Basic
          ...
          type={!CHART_TYPE_CONFIG[chartType].showTokens
            ...
            : <span>{t('appOverview.analysis.tokenUsage.consumed')} Tokens<span className='text-sm'>
              ...
              <span className='text-primary-600'>~{sum(statistics.map(item => parseFloat(get(item, 'total_price', '0')))).toLocaleString('en-US', { style: 'currency', currency: 'USD', minimumFractionDigits: 4 })}</span>
