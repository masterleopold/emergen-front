...
export const SimpleBtn = ({ className, isDisabled, onClick, children }: {
  ...
  <div
    className={cn(className, isDisabled ? 'border-gray-100 text-gray-300' : 'border-gray-300 text-gray-700 cursor-pointer hover:border-gray-300', 'flex items-center h-7 px-3 rounded-md border-[0.5px] text-xs  font-medium')}
...
const GenerationItem: FC<IGenerationItemProps> = ({
  ...
  const mainStyle = (() => {
    const res: React.CSSProperties = !isTop
      ? {
        background: depth % 2 === 0 ? 'linear-gradient(90.07deg, #FBFBFC 0.05%, rgba(249, 250, 251, 0) 99.93%)' : '#fff',
      }
      : {}
    ...
    <>
      ...
      {!isWorkflow && !isError && messageId && feedback?.rating === 'like' && (
        <div
          ...
          className='flex w-7 h-7 items-center justify-center rounded-md cursor-pointer  !text-primary-600 border-[0.5px] border-primary-200 bg-primary-100 hover:border-primary-300 hover:bg-primary-200'>
          <HandThumbUpIcon width={16} height={16} />
        </div>
      )}
      {!isWorkflow && !isError && messageId && feedback?.rating === 'dislike' && (
        <div
          ...
          className='flex w-7 h-7 items-center justify-center rounded-md cursor-pointer  !text-red-600 border-[0.5px] border-red-200 bg-red-100 hover:border-red-300 hover:bg-red-200'>
          <HandThumbDownIcon width={16} height={16} />
        </div>
      )}
    </>
  ...
  return (
    <div ref={ref} className={cn(className, isTop ? `rounded-xl border-[0.5px] ${!isError ? 'border-gray-300 bg-white' : 'border-[#FECDCA] bg-[#FEF3F2]'} ` : 'rounded-br-xl !mt-0')}
    >
      {isLoading
        ? (
        ...
        : (
          ...
          >
            {(isTop && taskId) && (
              <div className='mb-2 text-gray-500 border-[0.5px] border-gray-300 box-border flex items-center rounded-md italic text-[11px] pl-1 pr-1.5 font-medium w-fit group-hover:opacity-100'>
                ...
                {isInWebApp && (
                  <>
                    ...
                    {!isError && messageId && !isWorkflow && (
                      <div className="mx-3 w-[1px] h-[14px] bg-gray-300"></div>
                    )}
                    {ratingContent}
                  </>
                )}
