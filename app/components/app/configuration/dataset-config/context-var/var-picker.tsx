...
const VarItem: FC<{ item: Option }> = ({ item }) => (
  <div className='flex items-center h-[18px] px-1 bg-[#F8F4FE] rounded space-x-1'>
    <IconTypeIcon type={item.type as IInputTypeIconProps['type']} className='text-[#7C3AED]' />
    <div className='flex text-xs font-medium text-[#7C3AED]'>
    ...
    >
      <PortalToFollowElemTrigger className={cn(triggerClassName)} onClick={() => setOpen(v => !v)}>
        <div className={cn(
          s.trigger,
          className,
          notSetVar ? 'bg-[#FFFCF5] border-[#FEDF89] text-[#DC6803]' : ' hover:bg-gray-50 border-gray-300 text-primary-600',
          open ? 'bg-gray-50' : 'bg-white',
          `
          flex items-center h-8 justify-center px-2 space-x-1 rounded-lg border-[0.5px]  shadow-xs cursor-pointer
          ...
          <ChevronDownIcon className={cn(s.dropdownIcon, open && 'rotate-180 text-[#858F97]', 'w-3.5 h-3.5')} />
        </div>
      </PortalToFollowElemTrigger>
      <PortalToFollowElemContent style={{ zIndex: 1000 }}>
        {options.length > 0
          ? (<div className='w-[240px] max-h-[50vh] overflow-y-auto p-1  border-[0.5px] bg-white border-gray-300 rounded-lg shadow-lg'>
          ...
          : (
            <div className='w-[240px] p-6 bg-white border-[0.5px] border-gray-300 rounded-lg shadow-lg'>
