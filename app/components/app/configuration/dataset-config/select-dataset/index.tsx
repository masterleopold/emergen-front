    ...
    >
      ...
      {(loaded && hasNoData) && (
        ...
        >
          ...
          <Link href="/datasets/create" className='font-normal text-[#7C3AED]'>{t('appDebug.feature.dataSet.toCreate')}</Link>
        </div>
      )}
        ...
        <>
          <div ref={listRef} className='mt-7 space-y-1 max-h-[286px] overflow-y-auto'>
            {datasets.map(item => (
              <div
                key={item.id}
                className={cn(s.item, selected.some(i => i.id === item.id) && s.selected, 'flex justify-between items-center h-10 px-2 rounded-lg bg-white border-[0.5px] border-gray-300  cursor-pointer', !item.embedding_available && s.disabled)}
                onClick={() => {
              ...
              >
                <div className='mr-1 flex items-center'>
                  ...
                  {!item.embedding_available && (
                    <span className='ml-1 shrink-0 px-1 border-[0.5px] boder-gray-200 rounded-md text-gray-500 text-xs font-normal leading-[18px]'>{t('dataset.unavailable')}</span>
                  )}
                </div>
