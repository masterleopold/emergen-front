...
const ConfigVar: FC<IConfigVarProps> = ({ promptVariables, readonly, onPromptVariablesChange }) => {
  ...
  return (
    ...
    >
      ...
      {hasVar && (
        <div className='rounded-lg border-[0.5px] border-gray-300 bg-white overflow-x-auto'>
          <table className={`${s.table} min-w-[440px] w-full max-w-full border-collapse border-0 rounded-lg text-sm`}>
            <thead className="border-b-[0.5px]  border-gray-300 text-gray-500 text-xs font-medium">
            ...
            <tbody className="text-gray-700">
              {promptVariables.map(({ key, name, type, required, config, icon, icon_background }, index) => (
                <tr key={index} className="h-9 leading-9">
                  <td className="w-[160px] border-b-[0.5px] border-gray-300 pl-3">
                      ...
                      {!readonly
                        ? (
                          <input
                            ...
                            className="h-6 leading-6 block w-full rounded-md border-0 py-1.5 px-2 text-gray-900  placeholder:text-gray-400 focus:outline-none focus:ring-1 focus:ring-inset focus:ring-gray-200"
                          />
                        )
                        : (
                  ...
                  <td className="py-1 border-b-[0.5px] border-gray-300">
                    {!readonly
                      ? (
                        <input
                          type="text"
                  ...
                  {!readonly && (
                    <>
                      <td className='w-[84px] border-b-[0.5px] border-gray-300'>
                        <div className='flex items-center h-full'>
                          <Switch defaultValue={!required} size='md' onChange={value => updatePromptVariable(key, 'required', !value)} />
                        </div>
                      </td>
                      <td className='w-20  border-b-[0.5px] border-gray-300'>
                        <div className='flex h-full items-center space-x-1'>
                          <div className=' p-1 rounded-md hover:bg-gray-300 w-6 h-6 cursor-pointer' onClick={() => handleConfig({ type, key, index, name, config, icon, icon_background })}>
                            <Settings01 className='w-4 h-4 text-gray-500' />
                          </div>
                          <div className=' p-1 rounded-md hover:bg-gray-300 w-6 h-6 cursor-pointer' onClick={() => handleRemoveVar(index)} >
