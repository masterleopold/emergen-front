...
const SelectItem: FC<ItemProps> = ({ text, value, Icon, isChecked, description, onClick, disabled }) => {
  return (
    <div
      className={cn(disabled ? 'opacity-50' : 'cursor-pointer', isChecked ? 'border-[2px] border-primary-600' : 'border border-gray-100', 'mb-2 p-3 pr-4 rounded-xl bg-gray-25 hover:bg-gray-50')}
      onClick={() => !disabled && onClick(value)}
    >
      <div className='flex items-center justify-between'>
        <div className='flex items-center '>
          <div className='mr-3 p-1 bg-primary-50 rounded-lg'>
        ...
        <div className='flex items-center justify-between'>
          <div className='flex items-center '>
            <div className='mr-3 p-1 bg-gray-200 group-hover:bg-white rounded-lg'>
              <Settings04 className='w-4 h-4 text-gray-600 group-hover:text-[#7C3AED]' />
            </div>
            <div className='leading-5 text-sm font-medium text-gray-900 group-hover:text-[#7C3AED]'>{t('appDebug.agent.setting.name')}</div>
          </div>
          <ArrowUpRight className='w-4 h-4 text-gray-500 group-hover:text-[#7C3AED]' />
        </div>
    ...
    <>
      ...
      >
        <PortalToFollowElemTrigger onClick={() => setOpen(v => !v)}>
          <div className={cn(open && 'bg-gray-50', 'flex items-center h-8 px-3 border-[0.5px] border-gray-300 rounded-lg cursor-pointer select-none space-x-1 text-primary-600')}>
        ...
        <PortalToFollowElemContent style={{ zIndex: 1000 }}>
          <div className='relative left-0.5 p-6 bg-white border-[0.5px] border-black/8 shadow-lg rounded-xl w-[480px]'>
