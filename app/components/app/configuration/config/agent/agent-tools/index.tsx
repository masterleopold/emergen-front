      ...
      >
        <div className='grid gap-1 grid-cols-1 2xl:grid-cols-2 items-center flex-wrap justify-between'>
          {tools.map((item: AgentTool & { icon: any; collection?: Collection }, index) => (
            <div key={index}
              className={cn((item.isDeleted || item.notAuthor) ? 'bg-white/50' : 'bg-white', (item.enabled && !item.isDeleted && !item.notAuthor) && '', index > 1 && 'mt-1', 'group relative flex justify-between items-center last-of-type:mb-0  pl-2.5 py-2 pr-3 w-full  rounded-lg border-[0.5px] border-gray-300 ')}
            >
                ...
                {(item.isDeleted || item.notAuthor)
                  ? (
                    <div className='flex items-center'>
                      ...
                      >
                        <div className='mr-1 p-1 rounded-md hover:bg-gray-300  cursor-pointer' onClick={() => {
                          if (item.notAuthor)
                      ...
                      <div className='p-1 rounded-md hover:bg-gray-300 cursor-pointer' onClick={() => {
                        const newModelConfig = produce(modelConfig, (draft) => {
