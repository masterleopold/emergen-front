...
const PromptValuePanel: FC<IPromptValuePanelProps> = ({
  ...
  return (
    <div className="pb-3 border-[0.5px] border-gray-300 bg-white rounded-xl" style={{
      boxShadow: '0px 4px 8px -2px rgba(244, 245, 246, 0.1), 0px 2px 4px -2px rgba(244, 245, 246, 0.06)',
    }}>
          ...
          <div className='flex items-center space-x-1 cursor-pointer' onClick={() => setUserInputFieldCollapse(!userInputFieldCollapse)}>
            {
              userInputFieldCollapse
                ? <RiArrowRightLine className='w-3 h-3 text-gray-400' />
                : <RiArrowDownSLine className='w-3 h-3 text-gray-400' />
            }
          ...
          <>
                ...
                ? (
                  <div className="space-y-3 ">
                    {promptVariables.map(({ key, name, type, options, max_length, required }) => (
                      <div key={key} className="xl:flex justify-between items-center">
                        ...
                        {type === 'string' && (
                          <input
                            className="w-full px-3 text-sm leading-9 text-gray-900 border-0 rounded-lg grow h-11 bg-gray-200 focus:outline-none focus:ring-1 focus:ring-inset focus:ring-gray-200"
                        ...
                        {type === 'paragraph' && (
                          <textarea
                            className="w-full px-3 text-sm leading-9 text-gray-900 border-0 rounded-lg grow h-[120px] bg-gray-200 focus:outline-none focus:ring-1 focus:ring-inset focus:ring-gray-200"
                            placeholder={`${name}${!required ? `(${t('appDebug.variableTable.optional')})` : ''}`}
                            value={inputs[key] ? `${inputs[key]}` : ''}
                            onChange={(e) => { handleInputValueChange(key, e.target.value) }}
                          />
            ...
            }
            {
              appType === AppType.completion && visionConfig?.enabled && (
                <div className="mt-3 xl:flex justify-between items-center">
...

      {
        appType === AppType.completion && (
          <div>
            <div className="mt-5 border-b-[0.5px] border-gray-300"></div>
