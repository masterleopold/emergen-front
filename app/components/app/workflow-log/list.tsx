  ...
  return (
    <div className='overflow-x-auto'>
      <table className={`w-full min-w-[440px] border-collapse border-0 text-sm mt-3 ${s.logTable}`}>
        <thead className="h-8 !pl-3 py-2 leading-[18px] border-b-[0.5px] border-gray-300 text-xs text-gray-500 font-medium">
          <tr>
        ...
        <tbody className="text-gray-700 text-[13px]">
          {logs.data.map((log: WorkflowAppLogDetail) => {
            ...
            return <tr
              key={log.id}
              className={`border-b-[0.5px] border-gray-300 h-12 hover:bg-gray-100 cursor-pointer ${currentLog?.id !== log.id ? '' : 'bg-gray-50'}`}
              onClick={() => {
                setCurrentLog(log)
                setShowDrawer(true)
              }}>
              <td className='text-center align-middle'>{!log.read_at && <span className='inline-block bg-[#9058F0] h-1.5 w-1.5 rounded'></span>}</td>
      ...
      <Drawer
        ...
        mask={isMobile}
        footer={null}
        panelClassname='mt-16 mx-2 sm:mr-2 mb-3 !p-0 !max-w-[600px] rounded-xl border-[0.5px] border-gray-300'
      >
