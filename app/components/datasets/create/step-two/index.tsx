  ...
  return (
    <div className='flex w-full h-full'>
                    ...
                    <div className='w-full flex flex-col gap-1'>
                      ...
                      {rules.map(rule => (
                        <div key={rule.id} className={s.ruleItem}>
                          <input id={rule.id} type="checkbox" checked={rule.enabled} onChange={() => ruleChangeHandle(rule.id)} className="w-4 h-4 rounded border-gray-300 text-primary-700 focus:ring-primary-700" />
          ...
          <div className='max-w-[640px]'>
            ...
            {hasSetIndexType && (
              <div className='mt-2 text-xs text-gray-500 font-medium'>
                ...
                <Link className='text-[#7C3AED]' href={`/datasets/${datasetId}/settings`}>{t('datasetCreation.stepTwo.datasetSettingLink')}</Link>
              </div>
            )}
            {IS_CE_EDITION && indexType === IndexingType.QUALIFIED && (
              <div className='mt-3 rounded-xl bg-gray-200 border-[0.5px] border-gray-100'>
                <div className='flex justify-between items-center px-5 py-4'>
                  <div className='flex justify-center items-center w-8 h-8 rounded-lg bg-primary-50'>
                ...
                {docForm === DocForm.QA && !QATipHide && (
                  <div className='flex justify-between items-center px-5 py-2 bg-orange-50 border-t-[0.5px] border-amber-100 rounded-b-xl text-[13px] leading-[18px] text-medium text-amber-500'>
      ...
      <FloatRightContainer isMobile={isMobile} isOpen={showPreview} onClose={hidePreview} footer={null}>
        {showPreview && <div ref={previewScrollRef} className={cn(s.previewWrap, isMobile && s.isMobile, 'relative h-full overflow-y-scroll border-l-[0.5px] border-gray-300')}>
          <div className={cn(s.previewHeader, previewScrolled && `${s.fixed} pb-3`)}>
            ...
            {docForm === DocForm.QA && !previewSwitched && (
              <div className='px-8 pr-12 text-xs text-gray-500 pt-4'>
