const translation = {
  ...
  account: {
    ...
    langGeniusAccount: 'Emergenアカウント',
    langGeniusAccountTip: 'Emergenアカウントと関連するユーザーデータ。',
    editName: '名前を編集',
  ...
  members: {
    ...
    inviteTeamMemberTip: 'サインイン後、直接チームデータにアクセスできます。',
    email: 'メール',
    emailInvalid: '無効なメール形式',
    emailPlaceholder: 'メールを入力してください',
    sendInvite: '招待を送る',
    invitedAsRole: '{{role}}ユーザーとして招待',
    invitationSent: '招待が送信されました',
    invitationSentTip: '招待が送信され、Emergenにサインインしてチームデータにアクセスできます。',
  ...
  provider: {
    ...
    openaiHosted: {
      ...
      desc: 'Emergenが提供するOpenAIホスティングサービスを使用すると、GPT-3.5などのモデルを使用できます。トライアルクォータが使い果たされる前に、他のモデルプロバイダを設定する必要があります。',
      callTimes: '通話回数',
  ...
  apiBasedExtension: {
    title: 'API拡張機能は、Emergenのアプリケーション全体での簡単な使用のための設定を簡素化し、集中的なAPI管理を提供します。',
  ...
  about: {
    changeLog: '変更ログ',
    updateNow: '今すぐ更新',
    nowAvailable: 'Emergen {{version}} が利用可能です。',
    latestAvailable: 'Emergen {{version}} が最新バージョンです。',
  },
  appMenus: {
    overview: '概要',
    promptEng: 'オーケストレート',
  ...
  datasetMenus: {
    ...
    hitTesting: '検索テスト',
    settings: '設定',
    emptyTip: '関連付けられたナレッジがありません。アプリケーションやプラグインに移動して関連付けを完了してください。',
    viewDoc: 'ドキュメントを表示',
    relatedApp: '関連アプリ',
  },
  voiceInput: {
